...
extern char __dtb_z_dr_linkbox2p_revA_start[];
extern char __dtb_z_dr_linkbox2p_revB_start[];
extern char __dtb_z_dr_linkbox2p_revC_start[];
...
extern struct dram_timing_info dr_linkbox2p_dram_timing;
...
static void start_atf(void)
{
	...
	power_init_board();
	printf("Init DDR\n");
	imx8mp_ddr_init(&dr_linkbox2p_dram_timing, DRAM_TYPE_LPDDR4);
...
static int get_hwrev(void)
{
	...
	imx8mp_setup_pad(MX8MP_PAD_SAI1_TXD5__GPIO4_IO17 | INPUT_NOPULL_CTRL);
	imx8mp_setup_pad(MX8MP_PAD_SAI1_TXD6__GPIO4_IO18 | INPUT_NOPULL_CTRL);
	imx8mp_setup_pad(MX8MP_PAD_SAI1_TXD7__GPIO4_IO19 | INPUT_NOPULL_CTRL);

	imx8m_gpio_direction_input(gpiobase4, 17);
	imx8m_gpio_direction_input(gpiobase4, 18);
	imx8m_gpio_direction_input(gpiobase4, 19);

	ver |= imx8m_gpio_val(gpiobase4, 17);
	ver |= imx8m_gpio_val(gpiobase4, 18) << 1;
	ver |= imx8m_gpio_val(gpiobase4, 19) << 2;
 ...
 */
static __noreturn noinline void dr_linkbox2p_start(void)
{
	int hwrev;
	setup_uart();
	...
	switch (hwrev) {
	case 1:
		imx8mp_barebox_entry(__dtb_z_dr_linkbox2p_revA_start);
		break;

	case 0:
	case 2:
		imx8mp_barebox_entry(__dtb_z_dr_linkbox2p_revB_start);
		break;

	case 3:
	case 4:
		imx8mp_barebox_entry(__dtb_z_dr_linkbox2p_revC_start);
		break;
...
ENTRY_FUNCTION(start_dr_linkbox2p, r0, r1, r2)
{
